...
export default function ComplianceControl() {
  ...
  const sectionTitle = (title: string, icon: React.ReactNode) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ color: c.textPrimary, fontSize: 14, fontWeight: 600 }}>{title}</span>
    </div>
  );

  const statusBadge = (status: ApiEndpoint['status']) => {
    if (status === 'connected') return <Badge status="success" text={<span style={{ color: c.success, fontSize: 12 }}>已连接</span>} />;
    if (status === 'error') return <Badge status="error" text={<span style={{ color: c.danger, fontSize: 12 }}>连接失败</span>} />;
    return <Badge status="processing" text={<span style={{ color: c.warning, fontSize: 12 }}>重连中</span>} />;
  };
  ...
  // Compliance summary
  const passCount = complianceList.filter(item => item.status === 'pass').length;
  const warnCount = complianceList.filter(item => item.status === 'warning').length;
  const failCount = complianceList.filter(item => item.status === 'fail').length;
  const complianceScore = Math.round((passCount / complianceList.length) * 100);

  const agcColumns = [
    { title: '指令ID', dataIndex: 'id', width: 100, render: (v: string) => <span style={{ color: c.primary, fontSize: 12, fontFamily: 'monospace' }}>{v}</span> },
    { title: '时间', dataIndex: 'time', width: 90, render: (v: string) => <span style={{ color: c.textSecondary, fontSize: 12 }}>{v}</span> },
    {
    ...
    { title: '目标(MW)', dataIndex: 'targetMW', width: 90, render: (v: number) => <span style={{ color: c.textPrimary, fontSize: 12 }}>{v}</span> },
    {
  ...
  const complianceColumns = [
    { title: '类别', dataIndex: 'category', width: 90, render: (v: string) => <span style={{ color: c.textSecondary, fontSize: 12 }}>{v}</span> },
    { title: '检查项', dataIndex: 'item', render: (v: string) => <span style={{ color: c.textPrimary, fontSize: 13 }}>{v}</span> },
    {
      title: '状态', dataIndex: 'status', width: 80,
    ...
    { title: '说明', dataIndex: 'detail', render: (v: string) => <span style={{ color: c.textMuted, fontSize: 12 }}>{v}</span> },
    { title: '最近检查', dataIndex: 'lastCheck', width: 100, render: (v: string) => <span style={{ color: c.textDim, fontSize: 11 }}>{v}</span> },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Row gutter={16}>
        {[
          ...
          { label: '今日指令总量', value: cmdCount.toLocaleString(), unit: '条', color: c.primary, icon: <ClockCircleOutlined /> },
          ...
          <Col span={6} key={i}>
            <Card style={{ ...cardStyle, borderColor: `${kpi.color}22` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ color: c.textDim, fontSize: 12, marginBottom: 8 }}>{kpi.label}</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    <span style={{ color: kpi.color, fontSize: 28, fontWeight: 700, fontFamily: 'monospace' }}>{kpi.value}</span>
                    <span style={{ color: c.textDim, fontSize: 12 }}>{kpi.unit}</span>
                  </div>
                </div>
      ...
      <Card style={cardStyle} title={sectionTitle('API 接入状态', <ApiOutlined />)}>
        <Row gutter={16}>
          {apis.map(api => (
            <Col span={6} key={api.id}>
              <div style={{
                background: c.bgSider, borderRadius: 10, padding: 16,
                ...
                <div style={{ color: c.textPrimary, fontSize: 13, fontWeight: 600, marginBottom: 2 }}>{api.name}</div>
                <div style={{ color: c.textDim, fontSize: 11, marginBottom: 10 }}>{api.system}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: c.textDim, fontSize: 11 }}>响应延迟</span>
                    <span style={{ color: api.status === 'syncing' ? '#ffb800' : '#00ff88', fontSize: 11, fontFamily: 'monospace' }}>
                      {api.status === 'syncing' ? '—' : `${api.latency}ms`}
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: c.textDim, fontSize: 11 }}>今日调用</span>
                    <span style={{ color: c.textSecondary, fontSize: 11 }}>{api.todayCalls.toLocaleString()}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: c.textDim, fontSize: 11 }}>可用率</span>
                    <span style={{ color: api.uptime >= 99 ? '#00ff88' : api.uptime >= 97 ? '#ffb800' : '#ff4d4d', fontSize: 11 }}>{api.uptime}%</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: c.textDim, fontSize: 11 }}>最后同步</span>
                    <span style={{ color: c.textDim, fontSize: 11 }}>{api.lastSync}</span>
                  </div>
                </div>
                <Progress
                  percent={api.uptime}
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ color: c.textDim, fontSize: 12, marginBottom: 8 }}>AGC 接口响应时延（最近 30 分钟）</div>
          <ResponsiveContainer width="100%" height={100}>
            <LineChart data={latencyTrend} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="2 4" stroke={c.borderSubtle} />
              <XAxis dataKey="t" tick={{ fill: c.textDim, fontSize: 10 }} interval={9} />
              <YAxis tick={{ fill: c.textDim, fontSize: 10 }} domain={[0, 600]} />
              <Tooltip
                contentStyle={{ background: c.bgElevated, border: `1px solid ${c.primaryBorder}`, borderRadius: 6 }}
                labelStyle={{ color: c.textSecondary, fontSize: 11 }}
                itemStyle={{ color: c.primary, fontSize: 11 }}
                formatter={(v) => [`${v}ms`, '时延']}
              />
              <ReferenceLine y={500} stroke="#ff4d4d" strokeDasharray="4 4" label={{ value: '阈值500ms', fill: '#ff4d4d', fontSize: 10 }} />
              <Line type="monotone" dataKey="latency" stroke={c.primary} strokeWidth={1.5} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
        ...
        <Row gutter={16}>
          <Col span={16}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ color: c.textDim, fontSize: 12 }}>实时指令流（每15秒一条，自动刷新）</span>
              <div style={{ display: 'flex', gap: 8 }}>
                <Badge status="success" text={<span style={{ color: '#00ff88', fontSize: 12 }}>EMS穿透 已激活</span>} />
                <span style={{ color: c.primaryMuted }}>|</span>
                <Badge status="processing" color={c.primary} text={<span style={{ color: c.primary, fontSize: 12 }}>实时接收中</span>} />
              </div>
            </div>
          ...
          <Col span={8}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* 响应质量统计 */}
              <div style={{ background: c.bgSider, borderRadius: 10, padding: 16, border: `1px solid ${c.primaryMuted}` }}>
                <div style={{ color: c.textSecondary, fontSize: 12, marginBottom: 12 }}>今日响应质量</div>
                {[
                  ...
                  <div key={item.label} style={{ marginBottom: 10 }}>
                    ...
                    <Progress percent={item.value} showInfo={false} strokeColor={item.color} trailColor={c.borderSubtle} size="small" />
                  </div>
                ))}
              </div>
              ...
              {/* EMS 穿透状态 */}
              <div style={{ background: c.bgSider, borderRadius: 10, padding: 16, border: `1px solid ${c.primaryMuted}` }}>
                <div style={{ color: c.textSecondary, fontSize: 12, marginBottom: 12 }}>EMS 穿透通道</div>
                <Timeline
                  items={[
                    { color: '#00ff88', children: <span style={{ color: c.textPrimary, fontSize: 12 }}>调度中心 → VPP平台 <Tag color="success" style={{ fontSize: 10, marginLeft: 4 }}>激活</Tag></span> },
                    { color: '#00ff88', children: <span style={{ color: c.textPrimary, fontSize: 12 }}>VPP平台 → EMS系统 <Tag color="success" style={{ fontSize: 10, marginLeft: 4 }}>穿透</Tag></span> },
                    { color: '#00ff88', children: <span style={{ color: c.textPrimary, fontSize: 12 }}>EMS → 储能PCS <Tag color="success" style={{ fontSize: 10, marginLeft: 4 }}>下发</Tag></span> },
                    { color: '#ffb800', children: <span style={{ color: c.textSecondary, fontSize: 12 }}>PCS → 电池组 <Tag color="warning" style={{ fontSize: 10, marginLeft: 4 }}>部分</Tag></span> },
                  ]}
                />
              </div>

              {/* 响应时延 Gauge */}
              <div style={{ background: c.bgSider, borderRadius: 10, padding: 16, border: `1px solid ${c.primaryMuted}`, textAlign: 'center' }}>
                <div style={{ color: c.textDim, fontSize: 11, marginBottom: 8 }}>当前响应时延</div>
                <Statistic
                  value={liveLatency}
                  suffix="ms"
                  valueStyle={{ color: liveLatency < 300 ? '#00ff88' : '#ffb800', fontSize: 36, fontFamily: 'monospace' }}
                />
                <div style={{ color: c.textDim, fontSize: 11, marginTop: 4 }}>阈值要求：≤500ms</div>
                <Progress
      ...
      <Card style={cardStyle} title={sectionTitle('等保合规检查清单', <SafetyCertificateOutlined />)}>
        ...
        <Row gutter={16} style={{ marginBottom: 16 }}>
          {[
            ...
            { label: '合规得分', count: `${complianceScore}分`, color: complianceScore >= 90 ? '#00ff88' : '#ffb800', bg: c.primaryMuted },
          ].map((s, i) => (
            <Col span={6} key={i}>
        ...
        <div style={{ marginTop: 16, padding: '10px 16px', background: c.primaryMuted, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
          <LockOutlined style={{ color: c.primary }} />
          <span style={{ color: c.textDim, fontSize: 12 }}>
            等级保护三级认证状态：<span style={{ color: c.success }}>有效</span>（有效期至 2027-06-30）&nbsp;·&nbsp;
            下次全面检查：2026-06-15
          </span>
        </div>
      </Card>
