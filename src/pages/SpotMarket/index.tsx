...
export default function SpotMarket() {
  ...
  const backtestColumns: ColumnType<BacktestRow>[] = [
    { title: '日期', dataIndex: 'date', render: (v: string) => <span style={{ color: '#6b7280', fontSize: 12 }}>{v}</span> },
    {
      title: '策略', dataIndex: 'strategy',
      render: (v: string) => <Tag style={{ color: '#00d4ff', borderColor: '#00d4ff', background: 'rgba(0,212,255,0.1)' }}>{v}</Tag>,
    },
    { title: '充电量', dataIndex: 'chargeEnergy', render: (v: number) => <span style={{ color: '#aab4c8' }}>{v} MWh</span> },
    { title: '放电量', dataIndex: 'dischargeEnergy', render: (v: number) => <span style={{ color: '#aab4c8' }}>{v} MWh</span> },
    { title: '购电成本', dataIndex: 'buyCost', render: (v: number) => <span style={{ color: '#ff6b6b' }}>¥{(v / 10000).toFixed(1)}万</span> },
    { title: '售电收入', dataIndex: 'sellRevenue', render: (v: number) => <span style={{ color: '#00ff88' }}>¥{(v / 10000).toFixed(1)}万</span> },
    { title: '套利利润', dataIndex: 'profit', render: (v: number) => <span style={{ color: '#00ff88', fontWeight: 700 }}>¥{(v / 10000).toFixed(1)}万</span> },
    { title: '峰谷价差', dataIndex: 'roi', render: (v: number) => <span style={{ color: '#ffb800' }}>{v}x</span> },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <h2 style={{ color: '#00d4ff', margin: 0, fontSize: 20, letterSpacing: 1 }}>现货市场交易策略</h2>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.2)', borderRadius: 8, padding: '6px 12px' }}>
            ...
            <span style={{ color: '#00ff88', fontSize: 12 }}>市场开放</span>
          </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        {[
          { label: '当前谷段电价', value: `¥${valleyAvg}`, sub: '元/kWh', color: '#00ff88', icon: <FallOutlined /> },
          { label: '当前峰段电价', value: `¥${peakAvg}`, sub: '元/kWh', color: '#ff4d4d', icon: <RiseOutlined /> },
          { label: '峰谷价差', value: `¥${arbitrageSpread}`, sub: '元/kWh 套利空间', color: '#ffb800', icon: <ThunderboltOutlined /> },
          { label: '当前负荷', value: `${prices[new Date().getHours()]?.load ?? 820}`, sub: 'MW · 广东南网', color: '#00d4ff', icon: <ThunderboltOutlined /> },
          { label: '今日预计套利', value: `¥${Math.round(arbitrageSpread * capacity * 5 * 0.92 / 10) / 10}万`, sub: `${capacity}MW储能`, color: '#00ff88', icon: <CalculatorOutlined /> },
        ].map(item => (
      ...
      {activeTab === 'strategy' && (
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} lg={16}>
            <Card
              title={<span style={{ color: '#00d4ff' }}>24小时现货电价曲线（实时）</span>}
              style={cardStyle}
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  {Object.entries(PERIOD_CONFIG).map(([k, v]) => (
                    <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <span style={{ color: '#6b7280', fontSize: 11 }}>{v.label}</span>
                    </div>
                  ))}
                </div>
              }
            >
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={prices} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  {/* 充电区间着色 */}
                  <ReferenceArea x1="01:00" x2="06:00" fill="rgba(0,255,136,0.06)" label={{ value: '充电窗口', fill: '#00ff88', fontSize: 10, position: 'top' }} />
                  <ReferenceArea x1="14:00" x2="17:00" fill="rgba(255,77,77,0.06)" label={{ value: '放电窗口', fill: '#ff4d4d', fontSize: 10, position: 'top' }} />
                  <ReferenceArea x1="19:00" x2="21:00" fill="rgba(255,77,77,0.06)" />
                  {/* 平衡线 */}
                  <ReferenceLine y={0.542} stroke="rgba(170,180,200,0.3)" strokeDasharray="4 4" label={{ value: '平段基准', fill: '#6b7280', fontSize: 10 }} />
                  <XAxis dataKey="hour" stroke="#4a6080" tick={{ fontSize: 11 }} interval={2} />
                  <YAxis stroke="#4a6080" tick={{ fontSize: 11 }} tickFormatter={v => `¥${v}`} domain={[0.1, 1.4]} />
                  <Tooltip
                    contentStyle={{ background: '#1a2540', border: '1px solid #00d4ff', borderRadius: 8, fontSize: 12 }}
                  ...
                  <Area type="monotone" dataKey="forecast" name="预测电价" stroke="#00d4ff" fill="url(#forecastGrad)" strokeWidth={1.5} strokeDasharray="5 3" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
          </Col>
            ...
            <Card
              title={
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ color: '#00d4ff' }}>今日策略推荐</span>
                </div>
              }
              style={cardStyle}
              styles={{ header: headStyle }}
              ...
              {strategyWindows.map((w, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#6b7280', fontSize: 11 }}>{String(w.start).padStart(2, '0')}:00 — {String(w.end).padStart(2, '0')}:00</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600 }}>¥{w.price}</div>
                    <div style={{ color: '#4a6080', fontSize: 11 }}>元/kWh</div>
                  </div>
                </div>
              ))}

              <div style={{ marginTop: 12, padding: '10px 12px', background: '#0a0e1a', borderRadius: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ color: '#6b7280', fontSize: 12 }}>预计日套利收益</span>
                  <span style={{ color: '#00ff88', fontWeight: 700, fontSize: 16 }}>
                    ¥{Math.round(arbitrageSpread * capacity * 5 * 0.92 / 10) / 10}万
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#6b7280', fontSize: 12 }}>月度预期</span>
          ...
          {/* 分时段价格分布 */}
          <Col xs={24} md={12}>
            <Card title={<span style={{ color: '#00d4ff' }}>分时段电价分布</span>} style={cardStyle} styles={{ header: headStyle }}>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={priceDistData} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="period" stroke="#4a6080" tick={{ fontSize: 12 }} />
                  <YAxis stroke="#4a6080" tick={{ fontSize: 11 }} tickFormatter={v => `¥${v}`} />
                  <Tooltip
                    contentStyle={{ background: '#1a2540', border: '1px solid #00d4ff', borderRadius: 8, fontSize: 12 }}
          ...
          {/* 负荷曲线 */}
          <Col xs={24} md={12}>
            <Card title={<span style={{ color: '#00d4ff' }}>实时系统负荷曲线</span>} style={cardStyle} styles={{ header: headStyle }}>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={prices} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="hour" stroke="#4a6080" tick={{ fontSize: 11 }} interval={3} />
                  <YAxis stroke="#4a6080" tick={{ fontSize: 11 }} unit=" MW" />
                  <Tooltip contentStyle={{ background: '#1a2540', border: '1px solid #00d4ff', borderRadius: 8, fontSize: 12 }} />
      ...
      {activeTab === 'simulate' && (
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={10}>
            <Card
              title={<span style={{ color: '#00d4ff' }}><CalculatorOutlined /> 套利策略模拟计算器</span>}
              style={cardStyle} styles={{ header: headStyle }}
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div>
                  <div style={{ color: '#aab4c8', fontSize: 13, marginBottom: 8 }}>储能规模（MW）</div>
                ...
                <div>
                  <div style={{ color: '#aab4c8', fontSize: 13, marginBottom: 8 }}>充放电效率（%）</div>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <div>
                  <div style={{ color: '#aab4c8', fontSize: 13, marginBottom: 8 }}>充放策略</div>
                  <Radio.Group value={strategy} onChange={e => setStrategy(e.target.value)} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {[
                      ...
                      <div key={opt.value} onClick={() => setStrategy(opt.value as typeof strategy)}
                        style={{ padding: '10px 14px', borderRadius: 8, cursor: 'pointer', border: `1px solid ${strategy === opt.value ? '#00d4ff' : 'rgba(255,255,255,0.08)'}`, background: strategy === opt.value ? 'rgba(0,212,255,0.08)' : 'transparent' }}>
                        <div style={{ color: strategy === opt.value ? '#00d4ff' : '#e2e8f0', fontSize: 13, fontWeight: 600 }}>{opt.label}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    ...
                    <div key={item.label} style={{ background: '#0a0e1a', borderRadius: 6, padding: '8px 12px' }}>
                      <div style={{ color: '#4a6080', fontSize: 11 }}>{item.label}</div>
                      <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600 }}>{item.value}</div>
                    </div>
                  ))}
                </div>
                ...
                <Button
                  ...
                  style={{ background: '#00d4ff', border: 'none', color: '#0a0e1a', fontWeight: 700, borderRadius: 8, height: 44 }}
                >
                  运行模拟计算
                </Button>
              </div>
            ...
            {simResult ? (
              <Row gutter={[16, 16]}>
                ...
                {[
                  ...
                  { label: '月度预期', value: `¥${(simResult.profit * 25 / 10000).toFixed(0)}万`, color: '#00d4ff', desc: '按25个交易日' },
                  ...
                  <Col span={8} key={item.label}>
                    <Card style={{ ...cardStyle, border: `1px solid ${item.color}25` }} styles={{ body: { padding: '14px 16px' } }}>
                      <Statistic
                        title={<span style={{ color: '#6b7280', fontSize: 11 }}>{item.label}</span>}
                        value={item.value}
                        valueStyle={{ color: item.color, fontSize: 18, fontWeight: 700 }}
                      />
                      <div style={{ color: '#4a6080', fontSize: 11, marginTop: 4 }}>{item.desc}</div>
                    </Card>
                  </Col>
                ))}
                ...
                {/* 敏感性分析 */}
                <Col span={24}>
                  <Card title={<span style={{ color: '#00d4ff' }}>价差敏感性分析</span>} style={cardStyle} styles={{ header: headStyle }}>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart
                      ...
                      >
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                        <XAxis dataKey="价差" stroke="#4a6080" tick={{ fontSize: 11 }} />
                        ...
                        <Bar dataKey="日利润" fill="#00d4ff" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </Card>
                </Col>
              ...
              <Card style={{ ...cardStyle, height: '100%', minHeight: 400 }} styles={{ body: { height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 } }}>
                ...
                <div style={{ color: '#4a6080', fontSize: 14, textAlign: 'center' }}>
      ...
      {activeTab === 'forecast' && (
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} lg={16}>
            <Card title={<span style={{ color: '#00d4ff' }}>明日24小时电价预测（置信区间）</span>} style={cardStyle} styles={{ header: headStyle }}>
              <ResponsiveContainer width="100%" height={300}>
                ...
                >
                  ...
                  <Area type="monotone" dataKey="下界" stroke="transparent" fill="#0a0e1a" name="预测下界" />
                  <Line type="monotone" dataKey="预测中位" stroke="#00d4ff" strokeWidth={2.5} dot={false} name="预测中位价" />
                  <Line type="monotone" dataKey="上界" stroke="rgba(0,212,255,0.3)" strokeWidth={1} strokeDasharray="3 3" dot={false} name="90%置信上界" />
                  <Line type="monotone" dataKey="下界" stroke="rgba(0,212,255,0.3)" strokeWidth={1} strokeDasharray="3 3" dot={false} name="90%置信下界" />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
          </Col>
          ...
          <Col xs={24} lg={8}>
            <Card title={<span style={{ color: '#00d4ff' }}>关键时段预测</span>} style={cardStyle} styles={{ header: headStyle }}>
              {[
                ...
                <div key={i} style={{ padding: '10px 0', borderBottom: i < 4 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#aab4c8', fontSize: 12 }}>{item.period}</div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ color: '#4a6080', fontSize: 11 }}>风险：{item.risk}</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <Col span={24}>
            <Card title={<span style={{ color: '#00d4ff' }}>月度收益预测（2026年）</span>} style={cardStyle} styles={{ header: headStyle }}>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={monthlyForecast} margin={{ top: 4, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <YAxis stroke="#4a6080" tick={{ fontSize: 11 }} tickFormatter={v => `${(v / 10000).toFixed(0)}万`} />
                  <Tooltip contentStyle={{ background: '#1a2540', border: '1px solid #00d4ff', borderRadius: 8, fontSize: 12 }}
                    formatter={(v) => [`¥${(Number(v) / 10000).toFixed(1)}万`, '']} />
                  <Legend />
                  <Bar dataKey="套利收益" fill="#00d4ff" radius={[4, 4, 0, 0]} name="预测套利收益" />
        ...
        <Row gutter={[16, 16]}>
          <Col span={24}>
            <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
              {[
                { label: '回测周期', value: '近7天', color: '#00d4ff' },
                ...
                { label: '累计充电量', value: `${backtestData.reduce((a, b) => a + b.chargeEnergy, 0)} MWh`, color: '#aab4c8' },
              ].map(item => (
          ...
          <Col span={24}>
            <Card
              title={<span style={{ color: '#00d4ff' }}>历史套利交易记录</span>}
              style={cardStyle} styles={{ header: headStyle }}
              extra={
                  ...
                  <Table.Summary>
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell index={2}>
                        <span style={{ color: '#aab4c8' }}>{backtestData.reduce((a, b) => a + b.chargeEnergy, 0)} MWh</span>
                      </Table.Summary.Cell>
                      <Table.Summary.Cell index={3}>
                        <span style={{ color: '#aab4c8' }}>{backtestData.reduce((a, b) => a + b.dischargeEnergy, 0)} MWh</span>
                      </Table.Summary.Cell>
          ...
          <Col span={24}>
            <Card title={<span style={{ color: '#00d4ff' }}>每日套利收益走势</span>} style={cardStyle} styles={{ header: headStyle }}>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart
